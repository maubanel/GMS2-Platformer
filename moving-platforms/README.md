...
Since the background is moving it cannot be a background tile.  It has to be a game object for it to animate properly. Download [jumpPlatformSourceArt.png](images/jumpPlatformSourceArt.png). *Right click* on **Sprites** and select **New | Sprite** and name it `spr_platform`. Import the above image.
